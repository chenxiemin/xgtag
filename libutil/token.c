...
nexttoken(const char *interested, int (*reserved)(const char *, int))
{
	...
	char *p;
	int sharp = 0;
	int acceptSharp = 0; // do we first encounter # to read preprocessor
	int percent = 0;
	...
	while (EOF != (c = nextchar())) {
		...
		if (c == '"' || c == '\'') {	/* quoted string */
		...
		} else if (c & 0x80 || isalpha(c) || c == '_' || '~' == c) {
			p = token;
			if (acceptSharp) {
				*p++ = '#';
			} else if (percent) {
			...
			*p = 0;
			...
			if (sharp)
			{
				...
				{
					acceptSharp = 0;
					// ignore all of the unrecongnizable symbol
					message("Ignore symbol in sharp: %s at [+%d %s]", token, lineno, curfile);
				}
			}
			else
				break;
		} else {
			// if we are not under a line beginning with sharp
			if (sharp)
				message("Ignore special char %c because of sharp at %s %d", c, curfile, lineno);
			else
				if (interested == NULL || strchr(interested, c))
					break;
		}
