...
if !exists("s:Gtags_QueryFlag")
    let s:Gtags_QueryFlag = "--result=ctags -qe"
endif
" file to save gtags tmp output file folder
if !exists("g:Gtags_DefineSaveFolder")
    let g:Gtags_DefineSaveFolder = "/dev/shm"
endif
" file to save gtags tmp output file name
if !exists("s:Gtags_DefineSaveFile")
    let s:Gtags_DefineSaveFile = "GTAGSSAVE"
endif
...
function! s:ExecLoad(option, long_option, pattern)
    ...
    let l:option = l:option . ' --result=' . g:Gtags_Result . ' '
    ...
    let l:cmd = l:cmd . " | awk '{for (i=1; i<=NF; i++) if(i==1||i==2){ printf $i \":\" } else { printf $i \" \" } print \" \"; }'"
    " echo "option " . a:option . " long_option " . a:long_option . " pattern " . a:pattern . " l:cmd " . l:cmd

    let l:result = system(l:cmd)
    " let l:result = system("grep -n ab *.c")
    if v:shell_error != 0
        if v:shell_error != 0
            if v:shell_error == 2
...
" go to the define under cursor
function! GtagsDefine()
    let l:awkFormat = "awk '{print $2\":\"$3\":\"$1}'"
    " remove save file then write output tag into ti
    let l:fileSave = g:Gtags_DefineSaveFolder . '/' . s:Gtags_DefineSaveFile
    ...
    " prepare quickfix
    cgete l:result
    exec "tj " . expand("<cword>")
endfunction

" find the reference under cursor
function! GtagsRefers()
    " find definations
    let l:awkFormat = "awk '{for (i=1;i<=NF;i++) "
    let l:awkFormat = l:awkFormat. "if(i==1) printf(\"%s:\", $i); "
    ...
    let l:result = system(l:cmd)
    " find references
    " let l:cmd = s:global_command . " --result=ctags-mod -qrae " . expand("<cword>")
    " let l:result = l:result . system(l:cmd . " | " . l:awkFormat)
    " find symbols
    " let l:cmd = s:global_command . " --result=ctags-mod -qsae " . expand("<cword>")
    " let l:result = l:result . system(l:cmd . " | " . l:awkFormat)
    
    if '' == l:result
        call s:Error('Reference not found in ' . expand("<cword>"))
    else
        " open quick fix to show result
        cexpr! l:result
    endif
endfunction
...
if executable("xtag")
    autocmd BufWritePost * :silent! call s:AutoUpdateGtags()
endif
...
if g:Gtags_Auto_Map == 1
    " set tag file for tag jump
    :exec "set tags=" . g:Gtags_DefineSaveFolder . '/' . s:Gtags_DefineSaveFile
    :nmap <silent> <C-]> :call GtagsDefine()<cr>
    :nmap <silent> <C-\>s :call GtagsRefers()<CR>
endif
